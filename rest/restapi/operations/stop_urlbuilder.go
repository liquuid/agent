...
func (o *StopURL) Build() (*url.URL, error) {
	...
	var _path = "/rest/v1/stop/{container}"

	container := o.Container
	if container != "" {
		_path = strings.Replace(_path, "{container}", container, -1)
	} else {
		return nil, errors.New("Container is required on StopURL")
	}
...
func (o *StopURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on StopURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on StopURL")
	}
